...
public class PinnedSectionListView extends ListView {
  ...
  private final OnScrollListener mOnScrollListener = new OnScrollListener() {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
      ...
      final boolean isFirstVisibleItemSection = isItemViewTypePinned(adapter.getItemViewType(firstVisibleItem));
  ...
  private boolean isItemViewTypePinned(int value) {
    if (value == 1)
      return true;
    return false;
  }
  ...
  void createPinnedShadow(int position) {
    ...
    // measure & layout
    int ws = MeasureSpec
        .makeMeasureSpec(getWidth() - getListPaddingLeft() - getListPaddingRight(), MeasureSpec.EXACTLY);
  ...
  void ensureShadowForFirstItem(int sectionPosition, int firstVisibleItem, int visibleItemCount) {
    ...
    if (mPinnedSection != null && mPinnedSection.position != sectionPosition && sectionView.getTop() <= getPaddingTop()) {
      destroyPinnedShadow();
    }
  ...
  int findPreviousVisibleSectionPosition(int fromPosition) {
    ...
    for (int childIndex = fromPosition - 1; childIndex >= 0; childIndex--) {
      ...
      if (isItemViewTypePinned(viewType))
        return childIndex;
    }
    return -1;
  }
  ...
  int findCurrentSectionPosition(int fromPosition) {
    ...
    if (adapter instanceof SectionIndexer) {
      ...
      if (isItemViewTypePinned(typeView)) {
        return itemPosition;
      } // else, no luck
    }
  ...
  /**
   * Sets the selected item and positions the selection y pixels from the top edge of the ListView, or bottom edge of
   * the pinned view iff it exists. (If in touch mode, the item will not be selected but it will still be positioned
   * appropriately.)
   * 
   * @param position Index (starting at 0) of the data item to be selected.
   * @param y The distance from the top edge of the ListView (plus padding) that the item will be positioned.
