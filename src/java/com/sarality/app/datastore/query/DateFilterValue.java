...
/**
 * Converts a Date value into a String, based on the Column Spec so that it can be used
...
class DateFilterValue extends FilterValue<Date> {
  ...
  String getStringValue(Column column) {
    ...
    if (dataType != ColumnDataType.DATETIME) {
      throw new IllegalArgumentException("Cannot set filter value of type Date on column with data type "
          + dataType);
    }
