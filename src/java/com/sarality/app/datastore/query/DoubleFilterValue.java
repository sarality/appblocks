...
/**
 * Converts a double value into a String, based on the Column Spec so that it can be used
...
class DoubleFilterValue extends FilterValue<Double> {
  ...
  String getStringValue(Column column) {
    ...
    if (dataType == ColumnDataType.DOUBLE) {
      ...
      throw new IllegalArgumentException("Cannot set filter value of type Double on column with data type "
          + dataType);
    }
  }
}
